...
namespace YG
{
    ...
    public class LeaderboardYG : MonoBehaviour
    {
        ...
        [SerializeField] private Transform _rootSpawnPlayer;
        ...
        private void DestroyLBList()
        {
            ...
            int playerCount = _rootSpawnPlayer.childCount;
            for (int i = playerCount - 1; i >= 0; i--)
            {
                Destroy(_rootSpawnPlayer.GetChild(i).gameObject);
            }
        }
        ...
        private void SpawnPlayer(LBData lb)
        {
            for (int i = 0; i < players.Length; i++)
            {
                if (lb.players[i].uniqueID == YandexGame.playerId)
                {
                    GameObject playerObj = Instantiate(playerDataPrefab, _rootSpawnPlayer);
